   ...
   // Demand type
    ...
    public function post_addItem()
    {
       ...
       // Add project task link if needed
        ...
        //Si la entidad es igual a 1 (TI), enviar mensaje a telegram
        $entityId = $this->fields['entities_id'];
        $areaDeTiId = 1;

        if ($entityId == $areaDeTiId) {
            $this->sendTelegramNotification();	
        }
    }

    
    ///Envia notificación a Telegram por medio de un bot.
    ///Toma los campos del ticket, y extrae del título o descripción del ticket la información en caso de que 
    ///los datos del ticket solo estén almacenados en el títutlo y la descripción
    private function sendTelegramNotification(){
        ...
        // Datos básicos
        $telegramBotToken  = "7123018875:AAGhU4lQ3oIbFnDJz5tjTrxVlDd4inYMxAk";
        $telegramChatId    = "-1002535022845";
        $ticketId          = $this->fields['id'];
        $ticketTitle       = $this->fields['name'] ?? "Sin título";
        $username          = $_SESSION["glpiname"] ?? "anonymous";
        $ticketUrl         = "https://glpi.supermu.com/front/ticket.form.php?id=$ticketId"; 
	    $ticketDescription = $this->fields['content'];

        // Localización por defecto (se usa solo en la rama normal)
        $ticketLocalization = "Sin localización";
        if (!empty($this->fields['locations_id'])) {
            $loc = new Location();
            if ($loc->getFromDB($this->fields['locations_id'])) {
                $ticketLocalization = ucwords(strtolower($loc->fields['name']));
            }
        }

        // Categoría por defecto (se usa solo en la rama normal)
        $ticketCategory = "Sin categoría";
        if (!empty($this->fields['itilcategories_id'])) {
            $cat = new ITILCategory();
            if ($cat->getFromDB($this->fields['itilcategories_id'])) {
                // limpio números/puntos/guiones y normalizo
                $ticketCategory = trim(preg_replace('/[\d\-.]+/', '', $cat->fields['completename']));
                $ticketCategory = ucwords(strtolower($ticketCategory));
            }
        }
        $prefix = '[TK]sede:';

	if (strpos($ticketTitle, $prefix) === 0) {
    		$raw = substr($ticketTitle, strlen($prefix));

    $parts    = array_map('trim', explode(' - ', $raw));
    $rawNumber= array_pop($parts);

    if (count($parts) === 1) {
        list($tkSede, $tkCat) = array_map('trim', explode('-', $parts[0], 2));
    } else {
        $tkSede = $parts[0] ?: 'Sin ubicación';
        $tkCat  = end(array_filter($parts, fn($v) => $v !== '')) ?: 'Sin categoría';
    }

    // Normaliza mayúsculas/minúsculas
    $tkSede = ucwords(strtolower($tkSede));
    $tkCat  = ucwords(strtolower($tkCat));

    // Número (últimos 10 dígitos)
    $tkNumber = substr(preg_replace('/\D+/', '', $rawNumber), -10);
    
    if (preg_match('/Creado por:\s*(.+?)(?:[\r\n]|$)/i', $ticketDescription, $m)) {
    	$username = trim($m[1]);
    }

    // Construye el mensaje
    $message  = "🆔 *ID:* $ticketId\n";
    $message .= "👤 *Creado por:* $username\n";
    $message .= "📍 *Sede:* $tkSede\n"; 
    $message .= "🔢 *Número:* $tkNumber\n";
    $message .= "📂 *Categoría:* $tkCat\n";
    $message .= "🔗 [Ver ticket]($ticketUrl)";
}

        else {
            // ── Rama normal para tickets de GLPI
            $message  = "🎫 *Nuevo Ticket Creado*\n\n";
            $message .= "🆔 *ID:* $ticketId\n";
            $message .= "📌 *Título:* " . str_replace('_', '\_', $ticketTitle) . "\n";
            $message .= "👤 *Creado por:* $username\n";
            $message .= "📍 *Sede:* $ticketLocalization\n";
            $message .= "📂 *Categoría:* $ticketCategory\n";
            $message .= "🔗 [Ver ticket]($ticketUrl)";
        }
        ...
        // Log de resultado
        if ($response === false) {
            error_log("Error enviando notificación a Telegram para el ticket $ticketId.");
        } else {
            error_log("Notificación de Telegram enviada para el ticket $ticketId.");
        }
    }
